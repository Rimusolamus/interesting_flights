...
fun DetailScreen(
    ...
    Scaffold(
        topBar = { FlightDetailAppBar(title = "Bruges? Nah, flight to: " + state.flight.to, backToMain) },
    ) {
        FullScreenProgressBar(state.inProgress)

        if (!state.inProgress) {
            FlightDetailContent(state.flight)
        }
...
@Composable
private fun FlightDetailContent(flight: FlightDomain.FlightDomainItem) {
    Column(
        modifier = Modifier
            .fillMaxWidth()
            .padding(top = 48.dp),
        ...
        Text(
            text = "Detail Screen " + flight.toString(),
